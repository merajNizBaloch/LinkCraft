...
export default function QrScanner() {
  ...
  async function startCamera() {
    ...
    try {
      ...
      const controls = await reader.decodeFromVideoDevice(undefined, video, (decoded, _scanError, callbackControls) => {
        if (!decoded) return;
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-[28px] border border-[#d8d8d2] bg-white shadow-[0_24px_70px_rgba(17,17,15,.06)]">
      <div className="grid gap-0 lg:grid-cols-[1.05fr_.95fr]">
        <div className="border-b border-[#e2e2dc] p-5 sm:p-7 lg:border-b-0 lg:border-r">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <label className="secondary-action min-h-12 cursor-pointer">
              <Upload size={17} /> {scanningImage ? "Scanning image…" : "Upload QR image"}
              <input type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/bmp,image/*" className="sr-only" onChange={scanImage} disabled={scanningImage} />
            </label>
          </div>
          ...
          <div className="mt-3 flex items-start gap-2 rounded-xl bg-[#fafaf8] px-4 py-3 text-[11px] leading-5 text-[#777772]">
            ...
            <span>{fileName ? `Selected image: ${fileName}` : "For best results, keep the full QR code visible, well lit and reasonably sharp."}</span>
          </div>
        </div>
        ...
        <div className="p-5 sm:p-7">
          ...
          {error && (
            <div className="mt-6 rounded-2xl border border-[#ffd0c3] bg-[#fff0eb] p-4 text-sm font-semibold leading-6 text-[#a53419]">{error}</div>
          )}
          ...
          {!result && !error && (
            <div className="mt-6 grid min-h-52 place-items-center rounded-2xl border border-dashed border-[#d7d7d0] bg-[#fafaf8] p-6 text-center">
              <div>
                <div className="mx-auto grid h-11 w-11 place-items-center rounded-xl bg-[#fff0eb] text-[#ff5c35]"><QrResultIcon /></div>
          ...
          <div className="mt-6 rounded-2xl border border-[#deded8] bg-[#fafaf8] p-4 text-xs leading-5 text-[#777772]">
            Camera and uploaded images are processed in your browser for QR detection. Camera access only begins after you choose Start camera.
          </div>
        </div>
      </div>
    </div>
...
function QrResultIcon() {
  return <span className="text-lg font-black leading-none">QR</span>;
}
